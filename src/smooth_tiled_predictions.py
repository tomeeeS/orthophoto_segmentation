...
def predict_img_with_smooth_windowing(input_img, window_size, subdivisions, nb_classes,
                                      pred_func, aug=True):
    """
    ...
    if aug:
        ...
        for pad in tqdm(pads):
            # For every rotation:
            one_padded_result = tiled_predict(nb_classes, pad, pred_func, subdivisions, window_size)

            res.append(one_padded_result)
        # Merge after rotations:
        result = _rotate_mirror_undo(res)
    else:
        result = tiled_predict(nb_classes, pad, pred_func, subdivisions, window_size)
...
def tiled_predict(nb_classes, pad, pred_func, subdivisions, window_size):
    sd = _windowed_subdivs(pad, window_size, subdivisions, nb_classes, pred_func)
    one_padded_result = _recreate_from_subdivs(
        sd, window_size, subdivisions,
        padded_out_shape=list(pad.shape[:-1]) + [nb_classes])
    return one_padded_result
